...
class LSR_tensor_dot():
    ...
    def __init__(self, shape, ranks, separation_rank, dtype = np.float32, intercept = False ,initialize = True):
        ...
        self.init_params(intercept = intercept, initialize = initialize )
    ...
    def expand_to_tensor(self, skip_term = None):
        ...
        # Normalize factor matrices and core tensor
    
    # Normalize factor matrices and core tensor
    def normalize(self):
        # Expand to get the full tensor
        full_tensor = self.expand_to_tensor()
        
        # Calculate the norm of the reconstructed tensor
        tensor_norm = np.linalg.norm(full_tensor)

        # Compute the cubic root of the norm
        cubic_root_norm = tensor_norm ** (1/3)

        # Normalize the core tensor
        self.core_tensor /= cubic_root_norm

        # Normalize each factor matrix
        for s in range(self.separation_rank):
            for k in range(self.order):
                self.factor_matrices[s][k] /= cubic_root_norm
        print(f"Normalization complete- New Norm {np.linalg.norm(self.expand_to_tensor())}.")
